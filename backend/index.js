...
function findChromePath() {
  const chromePaths = [
    process.env.CHROME_EXECUTABLE_PATH, // Environment variable takes highest priority
    '/usr/bin/google-chrome',
    '/usr/bin/chromium-browser',
    '/usr/bin/chromium',
    '/opt/render/.cache/puppeteer/chrome/linux-132.0.6834.110/chrome-linux64/chrome',
    '/opt/render/chrome/chrome-linux/chrome', // Additional Render-specific path
  ...
  // If no path found, log all environment details
...
const launchBrowser = async () => {
  try {
    ...
    if (!chromePath) {
      throw new Error('No Chrome executable found. Please install Chrome or specify CHROME_EXECUTABLE_PATH.');
    }
    ...
    console.error('Detailed browser launch failure:', error);
    
    // Attempt to install Chromium if launch fails
    try {
      ...
      console.error('Chromium installation and launch failed:', installError);
      throw new Error(`Unable to launch browser: ${installError.message}`);
    }
  }
};
