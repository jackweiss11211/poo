...
function findChromePath() {
  ...
  for (const chromePath of chromePaths) {
    try {
      ...
      console.warn(`Chrome path check failed for: ${chromePath}`);
    }
  }
  return null;
}
...
const launchBrowser = async () => {
  try {
    ...
    // Attempt fallback launch without specifying executable path
    try {
      ...
      throw new Error(`Unable to launch browser: ${fallbackError.message}`);
    }
  }
};
