...
app.post('/search', validateSearchQuery, async (req, res) => {
  ...
  try {
    ...
    const results = await pornhub.search(query, ['link', 'title', 'download_urls']);
    ...
    for (const video of limitedResults) {
      if (video.download_urls) {
        // Prefer highest quality download URL
        const downloadUrl = Object.values(video.download_urls)[0];
        ...
        try {
          ...
          console.error(`Failed to download video: ${video.title}`, downloadError.message);
          failedVideos++;
        }
      }
    }
