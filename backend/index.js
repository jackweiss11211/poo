...
app.post('/search', validateSearchQuery, async (req, res) => {
  ...
  try {
    ...
    for (const video of limitedResults) {
      ...
      try {
        // Use the download URLs from the search results if available
        let downloadUrls = video.download_urls ? Object.values(video.download_urls) : [];

        // If no download URLs, skip this video
        if (downloadUrls.length === 0) {
          console.log(`No download URLs for video: ${video.title}`);
          failedVideos++;
          continue;
        }
